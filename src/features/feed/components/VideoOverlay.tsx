...
import {
  T, node, isType, children, findChoiceGroupAfter, getMenuForGroup, toSegment, resolveFromText
} from "../graphHelpers";

type Props = { organizationId: string; lessonId: string; onClose: () => void };

export default function VideoOverlay({ organizationId, lessonId, onClose }: Props) {
  const [preview, setPreview] = useState<any>(null);
  const [list, setList] = useState<any>(null);

  const [currentNodeId, setCurrentNodeId] = useState<string | null>(null); // nodo “timeline”
  const [segment, setSegment] = useState<{url:string;start:number;end?:number;poster?:string;title?:string} | null>(null);
  ...
  useEffect(() => {
    ...
    if (!v || !segment) return;
    const onLoaded = () => {
      try { v.currentTime = Math.max(0, segment.start ?? 0); } catch {}
      v.play().catch(() => {});
    };
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      if (segment.end != null && v.currentTime >= segment.end) {
        ...
        if (g) { setMenuId(g); setQuizId(null); return; }
  ...
  function onSelectOption(targetId: string) {
    ...
    if (isType(t, T.CHOICE_GROUP)) { // otro menú
      setMenuId(targetId);
      return;
    }
    if (isType(t, T.VIDEO)) {        // ir a clip de video
      goToVideoNode(targetId, t);
      return;
    }
    if (isType(t, T.JUMP)) {         // salto a lo que sea
      const dest = t.data?.jumpToNodeId;
      if (!dest) return;
      const d = node(list, dest);
      if (isType(d, T.CHOICE_GROUP)) { setMenuId(dest); return; }
      if (isType(d, T.VIDEO)) { goToVideoNode(dest, d); return; }
    }
    // fallback: busca algún video alcanzable desde ese id
    const q = [targetId]; const seen = new Set<string>();
    while (q.length) {
      const id = q.shift()!;
      if (seen.has(id)) continue; seen.add(id);
      const n = node(list, id); if (!n) continue;
      if (isType(n, T.VIDEO)) { goToVideoNode(id, n); return; }
      for (const k of children(n)) q.push(k);
    }
  }
  ...
  if (loading) return overlayRoot(<div style={{color:"#aaa"}}>Cargando…</div>, onClose);
    ...
    <>
      ...
      {baseUrl && (
        <video
          ...
          autoPlay
          playsInline
          style={{ width:"100%", height:"100%", maxWidth:900, background:"#000", borderRadius:12 }}
        />
      )}
          ...
          <div style={{display:"grid", gap:10, maxWidth:560}}>
            {menu.options.map(opt=>(
              <button key={opt.targetId} style={btn} onClick={()=>onSelectOption(opt.targetId)}>
                <PlayCircleIcon style={{width:20, height:20}} /> <span style={{overflow:"hidden",textOverflow:"ellipsis"}}>{opt.label}</span>
              </button>
            ))}
          </div>
        </div>
...
function overlayRoot(children: any, onClose: ()=>void) {
  return (
    <div style={root} role="dialog" aria-modal="true">
      <div style={topbar}>
        <button onClick={onClose} title="Cerrar" style={iconBtn}><XMarkIcon style={{width:28,height:28}}/></button>
      </div>
...
const root: React.CSSProperties = { position:"fixed", inset:0, background:"rgba(0,0,0,.98)", display:"flex", flexDirection:"column", zIndex:1000 };
const topbar: React.CSSProperties = { display:"flex", justifyContent:"flex-end", padding:10 };
const body: React.CSSProperties = { flex:1, display:"grid", placeItems:"center", padding:12, position:"relative" };
const iconBtn: React.CSSProperties = { background:"none", border:0, color:"#fff" };

const overlayBox: React.CSSProperties = {
  position:"absolute", inset:12, display:"flex", flexDirection:"column", gap:12,
  pointerEvents:"auto", justifyContent:"flex-start", alignItems:"center"
};
const overlayTitle: React.CSSProperties = { color:"#fff", fontWeight:700, textShadow:"0 2px 6px rgba(0,0,0,.6)" };
const btn: React.CSSProperties = {
  display:"flex", alignItems:"center", gap:12, background:"rgba(0,0,0,.45)", backdropFilter:"blur(4px)",
  border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:"12px 16px", color:"#fff", textAlign:"left"
};