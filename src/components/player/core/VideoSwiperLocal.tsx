...
export default function VideoSwiper({
  ...
  const playOnlyActiveSlide = (index: number) => {
    ...
    // 1) Arrancar SIEMPRE muteado (autoplay friendly)
    activeVideo.muted = true;
    // si estabas reseteando tiempo, dejalo si lo necesitás:
    // activeVideo.currentTime = 0;

    activeVideo
      .play()
      .then(() => {
        // 2) En cuanto esté corriendo, desmutear (bajo el gesto del swipe)
        // quitar atributo y subir volumen ayuda en iOS
        setTimeout(() => {
          try {
        ...
        }, 50); // pequeño delay: deja “enganchar” la reproducción
      })
      .catch(() => {
        // fallback: mantener muteado si el navegador no permite sonido aún
        try {
          activeVideo.muted = true;
    ...
    >
      ...
      <Swiper
        ...
        onSlideChange={(s) => {
          ...
          setResetToken((x) => x + 1);
          setTimeout(() => playOnlyActiveSlide(s.activeIndex), 0);
        }}
